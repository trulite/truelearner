...
fn main() {
    let mode = std::env::args().nth(1).unwrap_or_else(|| "--probe-v1".to_string());
    if mode == "--definitive" {
    ...
    println!("- Immutable old DS4 negative: `{}`", report.immutable_old_negative);
    println!("- Frozen protocol: `{}`", report.protocol_frozen);
    println!("- Physical arrival path: `{}`", report.physical_arrival_path);
    println!("- Learned event activity: `{}`", report.learned_event_activity);
    println!("- Occurrence selections: `{}`", report.occurrence_selections);
    println!("- Semantic feedback calls: `{}`", report.semantic_feedback_calls);
    println!("- M6 differential links: `{}`", report.m6_differential_links);
