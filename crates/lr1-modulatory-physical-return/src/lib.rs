#![forbid(unsafe_code)]
//! LR1 Arm C modulatory-transmission successor to PX0 CELL/ARROW/SPIKE physics.
//!
...
impl PlasticSubstrate {
    ...
    pub fn propagate(&mut self) -> Execution {
        ...
        while !self.pending.is_empty() {
            ...
            if mode == TransmissionMode::Modulatory {
                work.modulatory_deliveries += 1;
                self.apply_modulatory_return(spike.target, self.tick, &mut work);
                trace.push(TraceEntry {
                    tick: self.tick,
                    target_physical: self.cells[spike.target.0].physical_id,
                    impulse: spike.impulse,
                    fired: false,
                });
                continue;
            }
            work.drive_deliveries += 1;
    ...
    fn apply_modulatory_return(&mut self, cell: CellId, tick: i64, work: &mut WorkLedger) {
