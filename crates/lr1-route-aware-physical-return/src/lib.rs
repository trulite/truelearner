#![forbid(unsafe_code)]
//! LR1 Arm A route-aware successor to PX0 CELL/ARROW/SPIKE physics.
//!
...
pub struct WorkLedger {
    ...
    pub qualified_return_checks: u64,
    pub qualified_return_accepts: u64,
    pub qualified_return_path_edges: u64,
...
impl WorkLedger {
    pub fn total(&self) -> u64 {
        self.queue_comparisons
            ...
            + self.qualified_return_checks
            + self.qualified_return_accepts
            + self.qualified_return_path_edges
...
impl PlasticSubstrate {
    ...
    pub fn propagate(&mut self) -> Execution {
        ...
        while !self.pending.is_empty() {
            ...
            self.apply_qualified_local_return(
                spike.target,
                spike.arrow.map(|(arrow, _)| arrow),
                self.tick,
                &mut work,
            );
    ...
    pub fn arrow_eligible_until(&self, arrow: ArrowId) -> Option<i64> {
        self.require_arrow(arrow);
        self.arrows[arrow.0].eligible_until
    }
    ...
    fn apply_qualified_local_return(
        &mut self,
        cell: CellId,
        incoming: Option<ArrowId>,
        tick: i64,
        work: &mut WorkLedger,
    ) {
        ...
        let incoming_source = self.arrows[incoming.0].from;
        let eligible = self
            .arrows
            .iter()
            .enumerate()
            .filter_map(|(index, arrow)| {
                ...
                .then_some(ArrowId(index))
            })
            .collect::<Vec<_>>();

        for candidate in eligible {
            work.qualified_return_checks += 1;
            let candidate_target = self.arrows[candidate.0].to;
            let (qualified, edges) =
                self.has_recent_directed_path(candidate_target, incoming_source, tick);
            work.qualified_return_path_edges = work
                .qualified_return_path_edges
                .saturating_add(u64::try_from(edges).unwrap_or(u64::MAX));
            if !qualified {
                continue;
            }
            work.qualified_return_accepts += 1;
            let arrow = &mut self.arrows[candidate.0];
    ...
    fn has_recent_directed_path(&self, start: CellId, goal: CellId, tick: i64) -> (bool, usize) {
        if start == goal {
            return (false, 0);
        }
        ...
        while cursor < queue.len() {
            ...
            for arrow in &self.arrows {
                ...
                if !visited[arrow.to.0] {
                    visited[arrow.to.0] = true;
                    queue.push(arrow.to);
                }
            }
        }
