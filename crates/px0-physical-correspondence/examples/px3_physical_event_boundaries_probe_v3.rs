...
const V3_PROTOCOL_SHA256: &str = "2d36cd60a08c16abb41eb596dde3ddcfe70515731756f40cfdf665b2e774d4f9";
const V4_PROTOCOL_SHA256: &str = "fbb6661861b796bdaf4be5d93afd71b1ac42a09168dc2e6d3008c5bc996695f8";
...
const V2_SOURCE_SHA256: &str = "a15f2b1b5070d3fc707b68d0a4f7135834efbd9fc919e6a3c27d60f7751afad9";
const V2_RESULT_CSV_SHA256: &str =
    "5ae68dad569f943d08d945014ed5491a93eb30021b1afc78966ed39fd15b4cc4";
const V2_RESULT_MD_SHA256: &str =
    "941c1754004b43eee9a99b969bfbad9e1fd75257091fc475e4dbb196084eef66";

const V2_PROTOCOL: &str =
    "experiments/px3_physical_event_boundaries_no_new_mechanism_probe_v2_protocol.md";
const V3_PROTOCOL: &str =
    "experiments/px3_physical_event_boundaries_no_new_mechanism_probe_v3_protocol.md";
const PROTOCOL: &str =
    "experiments/px3_physical_event_boundaries_no_new_mechanism_probe_v4_protocol.md";
const RESULT_CSV: &str = "results/px3_physical_event_boundaries_no_new_mechanism_probe_v3.csv";
const RESULT_MD: &str = "results/px3_physical_event_boundaries_no_new_mechanism_probe_v3.md";
const STAGING_CSV: &str =
    "results/.px3_physical_event_boundaries_no_new_mechanism_probe_v3.csv.staging";
const STAGING_MD: &str =
    "results/.px3_physical_event_boundaries_no_new_mechanism_probe_v3.md.staging";
const NAMESPACE_A: u64 = 0x7_4300_0000;
const NAMESPACE_B: u64 = 0x7_5300_0000;
const NAMESPACE_BLOCKED: u64 = 0x7_6300_0000;
const NAMESPACE_SUBTHRESHOLD: u64 = 0x7_7300_0000;
const NAMESPACE_REPLICA_A: u64 = 0x8_4300_0000;
const NAMESPACE_REPLICA_B: u64 = 0x8_5300_0000;
...
fn main() {
    ...
    if preflight {
        println!("PX3_PHYSICAL_EVENT_BOUNDARIES_NO_NEW_MECHANISM_PROBE_V3_PREFLIGHT_OK");
        return;
    }

    eprintln!("PX3_PHYSICAL_EVENT_BOUNDARIES_NO_NEW_MECHANISM_PROBE_V3_EVIDENCE_SPENT");
    let report = run_probe();
    write_atomic(&report);
    println!(
        "PX3_PHYSICAL_EVENT_BOUNDARIES_NO_NEW_MECHANISM_PROBE_V3_{}",
...
fn run_cell(
    ...
    let first_tick = physics::clock(&matter) + 16;
    ...
    let post_gap = if recurrences == 1 { 70 } else { POST_GAP };
...
fn equivalent_state(left: &physics::State, right: &physics::State) -> bool {
    let mut left_directional = left.directional_resistance;
    let mut right_directional = right.directional_resistance;
    left_directional.sort_unstable();
    right_directional.sort_unstable();
    let mut left_live = left.directional_live;
    let mut right_live = right.directional_live;
    left_live.sort_unstable();
    right_live.sort_unstable();
    left.correspondence_resistance == right.correspondence_resistance
        && left_directional == right_directional
...
fn source_audit() -> bool {
    ...
    let source = include_str!("px3_physical_event_boundaries_probe_v3.rs");
    let physical = source
...
fn write_atomic(report: &Report) {
    ...
    let markdown = format!(
        "# PX3 physical event-boundary no-new-mechanism PROBE v3\n\nVerdict: **{}**.\n\nThis is a development PROBE, not an authoritative or definitive result. PX3 remains absent.\n\n| clause | result |\n|---|:---:|\n| frozen source and parent audit | {} |\n| matched per-route participation marginals | {} |\n| A/B relation-normalized persistent route state equal | {} |\n| trained/crossed held-out behavior symmetric | {} |\n| transient close/gap signal visible | {} |\n| recurrence, blocked-return, layout, replay, quiescence controls | {} |\n| target emerges without new mechanism | {} |\n\nCells: `{}` positive-history/layout cells plus blocked-return and subthreshold controls. Total ledgered work: `{}` operations.\n\n## Interpretation\n\nThe retained PX0–PX2 physics preserved transient timing, matured every actually traversed and returned route, survived the fixed post-gap, and naturally quiesced with zero autonomous source refiring. Matched recurring partitions left the same relation-free route topology and resistance multiset, and a trained pair was physically indistinguishable from a crossed pair during held-out use. Exact per-route values and fingerprints remain serialized; their early/late phase residue does not relate two routes or change held-out execution. The shared PX2 return hub records route participation marginals, not which routes recurred together.\n\nTransient clustering therefore does not become reusable event organization in existing PX0–PX2 state. Repair would require relation-specific persistent structure or a new substrate law. Both exceed this lane's authority, so there is no MICRO or GATE.\n",
        report.classification,
        report.source_exact,
