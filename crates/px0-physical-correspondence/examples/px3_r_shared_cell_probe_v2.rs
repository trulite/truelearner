...
const V1_SOURCE_SHA256: &str = "2268c4445b438ae8e3d4bd6e1cbdc93d5d217c39b0d8200ac1c0a4d8d7f61e4c";
const V1_RESULT_CSV_SHA256: &str =
    "1222b90fee4ed0ce2a20db7ea751f4c469073cde854b1b8ca0e774f9faf8038a";
const V1_RESULT_MD_SHA256: &str =
    "d7e559e4ebe434e236e8af3e08d88843dc6a4761ba879d5984452a78bb658973";
const PROTOCOL_SHA256: &str = "59c65606ad792a6b0c78e3f3d4a829eb69d77364f39df26a33edda08959fe844";

const PROTOCOL: &str = "experiments/px3_r_shared_cell_probe_v2_protocol.md";
const RESULT_CSV: &str = "results/px3_r_shared_cell_probe_v2.csv";
const RESULT_MD: &str = "results/px3_r_shared_cell_probe_v2.md";
const STAGING_CSV: &str = "results/.px3_r_shared_cell_probe_v2.csv.staging";
const STAGING_MD: &str = "results/.px3_r_shared_cell_probe_v2.md.staging";

const NAMESPACE_MAIN: u64 = 0x9_B110_0000;
const NAMESPACE_REPLICA: u64 = 0x9_B210_0000;
const NAMESPACE_SPACING: u64 = 0x9_B310_0000;
const NAMESPACE_ALTERNATIVE: u64 = 0x9_B410_0000;
const NAMESPACE_CORRELATED: u64 = 0x9_B510_0000;
const NAMESPACE_BLOCKED: u64 = 0x9_B610_0000;
const NAMESPACE_ABSENT: u64 = 0x9_B710_0000;
const NAMESPACE_STALE: u64 = 0x9_B810_0000;
const NAMESPACE_AMBIGUOUS: u64 = 0x9_B910_0000;
const NAMESPACE_MULTIPLE: u64 = 0x9_BA10_0000;
...
mod physics {
    ...
    pub(super) fn fresh(
        ...
        for site in site_order {
            ...
            locals[site] =
                Some(substrate.add_cell(cell(namespace + 200 + site as u64, center, 0, 4, 1_000)));
        ...
        for site in 0..SITES {
            for side in 0..2 {
                ...
                if weak_present {
                    substrate.add_arrow(arrow(sources[port], locals[site], 1, 2, 1));
                }
            }
...
fn main() {
    ...
    if preflight {
        println!("PX3_R_SHARED_CELL_PROBE_V2_PREFLIGHT_OK");
        return;
    }

    eprintln!("PX3_R_SHARED_CELL_PROBE_V2_EVIDENCE_SPENT");
    let report = run_probe();
    write_atomic(&report);
    println!("PX3_R_SHARED_CELL_PROBE_V2_{}", report.classification);
    if report.positive {
        std::process::exit(0);
    }
    std::process::exit(1);
...
fn source_audit() -> bool {
    let hashes = [
        ...
        (
            "results/px3_r_shared_cell_probe_v1.csv",
            V1_RESULT_CSV_SHA256,
        ),
        ("results/px3_r_shared_cell_probe_v1.md", V1_RESULT_MD_SHA256),
    ...
    .as_deref()
        ...
        && command_output(&[
            "rev-parse",
            "px3-r-shared-cell-probe-v1-first-clause-failure^{commit}",
        ])
        .as_deref()
            == Some("74813a268f593340703f1fbe4510de57e3c25276")
        && Command::new("git")
    ...
    let isolated = changed_paths.lines().all(|path| {
        path.starts_with("experiments/px3_r_shared_cell_probe_")
            || path.starts_with("results/px3_r_shared_cell_probe_")
            || path
                .starts_with("crates/px0-physical-correspondence/examples/px3_r_shared_cell_probe_")
    });
    hashes_exact && ancestry_exact && isolated && forbidden_audit()
}

fn forbidden_audit() -> bool {
    let source = include_str!("px3_r_shared_cell_probe_v2.rs");
    let physical = source
