#![forbid(unsafe_code)]
//! LR1 Arm B compartmental-return successor to PX0 CELL/ARROW/SPIKE physics.
//!
...
impl PlasticSubstrate {
    ...
    pub fn propagate(&mut self) -> Execution {
        ...
        while !self.pending.is_empty() {
            ...
            self.apply_compartmental_return(
                spike.target,
    ...
    fn apply_compartmental_return(
        ...
        let compartment_position = self.cells[compartment.0].position;
        let adjacent_sources = self
            .cells
            .iter()
            .enumerate()
            .filter_map(|(index, cell)| {
                (CellId(index) != compartment
                    && cell.live
                    && cell.position.saturating_sub(compartment_position).abs() == 1)
                .then_some(CellId(index))
            })
            .collect::<Vec<_>>();
        work.qualified_return_path_edges = work
            .qualified_return_path_edges
            .saturating_add(u64::try_from(adjacent_sources.len()).unwrap_or(u64::MAX));
