use organism_v0::post_m7_ds5_closure_emission_definitive::{
...
const CSV_PATH: &str = "results/post_m7_ds5_closure_emission_definitive.csv";
const MD_PATH: &str = "results/post_m7_ds5_closure_emission_definitive.md";
const CSV_STAGE: &str = "results/.post_m7_ds5_closure_emission_definitive.csv.staging";
const MD_STAGE: &str = "results/.post_m7_ds5_closure_emission_definitive.md.staging";
...
fn main() {
    ...
    match args.as_slice() {
        ...
        _ => {
            eprintln!("usage: post_m7_ds5_closure_emission_definitive [--audit|--definitive]");
            std::process::exit(2);
        }
    }
}
...
mod tests {
    ...
    fn temporary_paths() -> (PathBuf, PathBuf, PathBuf) {
        let directory = std::env::temp_dir().join(format!(
            "post-m7-ds5-definitive-publication-refusal-{}",
            std::process::id()
        ));
