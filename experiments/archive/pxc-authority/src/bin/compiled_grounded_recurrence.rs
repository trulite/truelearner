...
use organism_v0::reflected_program_discovery::grounding::compiled_recurrence::{
    print_rc0a_report, rc0a_csv, rc0a_markdown, run_rc0a_harness,
};
...
fn main() {
    ...
    if mode != HarnessMode::Definitive {
        ...
        let report = run_rc0a_harness(mode);
        print_rc0a_report(&report);
    ...
    let csv = arguments
        .first()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("results/rc0a_compiled_grounded_recurrence.csv"));
    let markdown = arguments
        .get(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("results/rc0a_compiled_grounded_recurrence.md"));
    ...
    let report = run_rc0a_harness(mode);
    print_rc0a_report(&report);
    write_new(&csv, &rc0a_csv(&report));
    write_new(&markdown, &rc0a_markdown(&report));
