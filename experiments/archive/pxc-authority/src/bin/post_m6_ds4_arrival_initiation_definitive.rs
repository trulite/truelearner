use organism_v0::post_m6_ds4_arrival_initiation_definitive::{
...
const CSV_PATH: &str = "results/post_m6_ds4_arrival_initiation_definitive.csv";
const MD_PATH: &str = "results/post_m6_ds4_arrival_initiation_definitive.md";
const CSV_STAGE: &str = "results/.post_m6_ds4_arrival_initiation_definitive.csv.staging";
const MD_STAGE: &str = "results/.post_m6_ds4_arrival_initiation_definitive.md.staging";
...
fn main() {
    ...
    match args.as_slice() {
        ...
        _ => {
            eprintln!("usage: post_m6_ds4_arrival_initiation_definitive [--audit|--definitive]");
            std::process::exit(2);
        }
    }
}
...
mod tests {
    ...
    fn temporary_paths() -> (PathBuf, PathBuf, PathBuf) {
        let directory = std::env::temp_dir().join(format!(
            "post-m6-ds4-definitive-publication-refusal-{}",
            std::process::id()
        ));
        fs::create_dir(&directory).expect("fresh bounded test directory is created once");
