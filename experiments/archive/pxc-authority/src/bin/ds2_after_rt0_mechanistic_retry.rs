...
fn main() {
    ...
    if arg == "--definitive" {
        eprintln!("DS2 after RT0: definitive execution is forbidden");
        std::process::exit(2)
    }
    if arg != "--probe" {
        eprintln!("usage: ds2_after_rt0_mechanistic_retry [--probe]");
        std::process::exit(2)
    }
    let report = retry::run();
    println!(
        "protocol={} claim_eligible={} audit={} M1_authoritative={} M2_exists={} RT0_seeds={} retained={} reversed={} first_collapse_stage={:?} first_collapse={} source={:?}",
        report.protocol,
        report.claim_eligible,
        report.audit_passed,
        report.m1_authoritative,
        report.m2_exists,
        report.rt0_seed_count,
        report.retained_contexts,
    ...
    println!(
        "lineage parent={} protocol_commit={} M1={} prior_retry={} RT0={} RT0_readiness={} protocol_hash={}",
        retry::EXACT_PARENT,
        retry::PROTOCOL_COMMIT,
        retry::AUTHORITATIVE_M1,
        retry::FROZEN_PRIOR_RETRY_SHA256,
        retry::FROZEN_RT0_SHA256,
        retry::FROZEN_RT0_READINESS_SHA256,
