...
fn main() {
    ...
    if arg == "--definitive" {
        eprintln!("DS2 after AP0: definitive execution is forbidden");
        std::process::exit(2)
    }
    if arg != "--probe" {
        eprintln!("usage: ds2_after_ap0_mechanistic_retry [--probe]");
        std::process::exit(2)
    }
    let report = retry::run();
    println!(
        "protocol={} claim_eligible={} audit={} M1_authoritative={} M2_exists={} AP0_seeds={} selected_proposals={} selected_support={} alternate_proposals={} alternate_support={} first_collapse_stage={:?} first_collapse={} source={:?}",
        report.protocol,
        report.claim_eligible,
        report.audit_passed,
        report.m1_authoritative,
        report.m2_exists,
        report.ap0_seed_count,
        report.selected_proposals,
        report.selected_support_updates,
        report.alternate_proposals,
    ...
    println!(
        "lineage parent={} protocol_commit={} M1={} prior_retry={} AP0={} AP0_readiness={} protocol_hash={}",
        retry::EXACT_PARENT,
        retry::PROTOCOL_COMMIT,
        retry::AUTHORITATIVE_M1,
        retry::FROZEN_PRIOR_RETRY_SHA256,
        retry::FROZEN_AP0_SHA256,
        retry::FROZEN_AP0_READINESS_SHA256,
