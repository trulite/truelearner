...
fn audit() {
    for (path, expected) in [
        ...
        (
            "arms/px3-r5-three-factor-return-attribution/src/main.rs",
            R5_SOURCE,
        ),
        (
            "results/px3_r5_three_factor_return_attribution_v1.csv",
            R5_CSV,
        ),
        (
            "results/px3_r5_three_factor_return_attribution_v1.md",
            R5_REPORT,
        ),
        (
            "experiments/px3_r5_three_factor_return_attribution_result_audit_v1.md",
            R5_AUDIT,
        ),
        (
            "experiments/px3_r6_return_triggered_trace_readout_protocol_v1.md",
            PROTOCOL,
        ),
        (
            "experiments/px3_r6_return_triggered_trace_readout_execution_protocol_v1.md",
            EXECUTION_PROTOCOL,
        ),
    ] {
...
fn replay(seed: u64, kind: Kind) -> Row {
    ...
    row.passed =
        row.observation.validity.into_iter().all(|value| value) && row.observation.claim_pass;
    row
}
...
fn schedule_valid(
    ...
    match kind {
        ...
        Kind::AdjacentCurrentReturn => factors(
            primitive,
            primitive_trace,
            p,
            x,
...
fn factors(
    ...
    expected_p: &[i64],
    expected_x: &[i64],
    expected_p_trace: &[i64],
    expected_x_trace: &[i64],
    ...
    primitive[0] == expected_primitive
        ...
        && p == expected_p
        && x == expected_x
        && p_trace == expected_p_trace
        && x_trace == expected_x_trace
...
fn csv(rows: &[Row]) -> String {
    ...
    for row in rows {
        ...
        let fields = vec![
            ...
            join_nested(&v.primitive),
            join_nested(&v.primitive_trace),
            join_i64(&v.p_fires),
            join_i64(&v.x_fires),
            join_i64(&v.p_trace),
            join_i64(&v.x_trace),
            join_i64(&v.return_trace),
            join_string(&v.p_foot_events),
            join_string(&v.x_foot_events),
            join_i64(&v.p_foot_fires),
            join_i64(&v.x_foot_fires),
            join_string(&v.m_events),
            join_i64(&v.m_fires),
            join_string(&v.echo),
            join_string(&v.candidate),
            v.candidate_resistance
                .iter()
                .map(u32::to_string)
                .collect::<Vec<_>>()
                .join("|"),
            v.candidate_liveness
                .iter()
                .map(bool::to_string)
                .collect::<Vec<_>>()
                .join("|"),
            v.return_updates.to_string(),
            v.proposals.to_string(),
            v.validity[0].to_string(),
            v.validity[1].to_string(),
            v.validity[2].to_string(),
            v.validity[3].to_string(),
            v.validity[4].to_string(),
            v.validity
                .into_iter()
                .filter(|item| *item)
                .count()
                .to_string(),
            v.claim_pass.to_string(),
            v.work.to_string(),
            v.bytes.to_string(),
            v.fingerprint.to_string(),
            v.permanent.to_string(),
            v.quiescent.to_string(),
...
fn report(rows: &[Row]) -> String {
    ...
    let repeated_m_arrivals = repeated
        .clone()
        .map(|row| row.observation.m_events.len())
        .sum::<usize>();
    let repeated_m_fires = repeated
        .map(|row| row.observation.m_fires.len())
        .sum::<usize>();
...
fn publish(stage: &str, destination: &str, content: &str) {
    ...
    rename(stage, destination).expect("publish");
}
...
mod tests {
    ...
    #[test]
    fn repeated_control() {
        assert!(Kind::ALL.contains(&Kind::RepeatedNoReturn));
    }
}
