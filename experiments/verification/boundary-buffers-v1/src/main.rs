...
const AUTHORITY_CASES: [Case; 16] = [
    Case::new(4_200_001, false, false, 1_040),
    Case::new(4_200_002, false, false, 1_170),
    Case::new(4_200_003, false, false, 1_300),
    Case::new(4_200_004, false, false, 1_430),
    Case::new(4_200_005, true, false, 1_040),
    Case::new(4_200_006, true, false, 1_170),
    Case::new(4_200_007, true, false, 1_300),
    Case::new(4_200_008, true, false, 1_430),
    Case::new(4_200_009, false, true, 1_040),
    Case::new(4_200_010, false, true, 1_170),
    Case::new(4_200_011, false, true, 1_300),
    Case::new(4_200_012, false, true, 1_430),
    Case::new(4_200_013, true, true, 1_040),
    Case::new(4_200_014, true, true, 1_170),
    Case::new(4_200_015, true, true, 1_300),
    Case::new(4_200_016, true, true, 1_430),
];
...
impl Mode {
    ...
    fn cases(self) -> [Case; 16] {
        assert_eq!(self, Self::Authority);
        AUTHORITY_CASES
    }

    fn csv(self) -> &'static str {
        assert_eq!(self, Self::Authority);
        "experiments/results/boundary_buffers_v1_authority.csv"
    }

    fn report(self) -> &'static str {
        assert_eq!(self, Self::Authority);
        "experiments/results/boundary_buffers_v1_authority.md"
    }
    ...
    fn title(self) -> &'static str {
        assert_eq!(self, Self::Authority);
        "Boundary Buffers V1 successor authority"
    }
}
...
fn main() {
    ...
    if mode == Mode::Preflight {
        ...
        assert!(cases
            .windows(2)
            .all(|pair| pair[0].root.saturating_add(1) == pair[1].root));
        assert!(cases.iter().all(|case| case.origin.rem_euclid(10) == 0));
        println!("{}", mode.marker());
        return;
    }
    ...
    assert!(
        ...
        "physical body clause failed"
    );
    assert!(
        buffers.clauses.into_iter().all(|value| value),
        "boundary buffer clause failed"
    );
    println!("BOUNDARY_BUFFERS_V1_AUTHORITY_ESTABLISHED rows=16/16 clauses=548/548");
}
...
fn run(case: Case, mechanics: MechanicalConfig, physical_trace: bool) -> Trial {
    ...
    let mut space =
        BoundaryRuntime::new(space, OUTWARD_REGION, INPUT_CAPACITY, OUTPUT_CAPACITY).unwrap();

    let blank = Batch::from(space.arrive(&[], OUTWARD_REGION).unwrap());
    ...
    batches.push(Batch::from(space.arrive(&inputs, OUTWARD_REGION).unwrap()));
...
fn add_execution_cost(total: &mut ExecutionCost, value: ExecutionCost) {
    ...
    total.frontier_samples = total
        .frontier_samples
        .saturating_add(value.frontier_samples);
    ...
    total.eligible_frontier_max = total.eligible_frontier_max.max(value.eligible_frontier_max);
    ...
    for (total_bucket, value_bucket) in total.batch_histogram.iter_mut().zip(value.batch_histogram)
    {
...
fn globals(mode: Mode, rows: &[Row], pxr0_exact: bool) -> [bool; 12] {
    let roots = rows
        ...
        .collect::<BTreeSet<_>>();
    assert_eq!(mode, Mode::Authority);
    let expected_start = 4_200_001;
    ...
    let expected_origins = [1_040, 1_170, 1_300, 1_430];
    ...
    let harness_gate = std::fs::read_to_string(
        "experiments/archive/pxc-authority/results/pxc_harness_audit_v1/audit.json",
    )
    .unwrap_or_default();
    let active_gate = std::fs::read_to_string(
        "experiments/archive/pxc-authority/results/pxc_active_gate_v1/audit.json",
    )
    .unwrap_or_default();
    let execution_gate = std::fs::read_to_string(
        "experiments/archive/pxc-authority/results/pxc_authority_firewall_v1/audit.json",
    )
    .unwrap_or_default()
...
fn markdown(
    ...
    let body_clauses = body.clauses.iter().filter(|value| **value).count();
    let buffer_clauses = buffers.clauses.iter().filter(|value| **value).count();
    ...
    writeln!(text, "- cumulative global clauses: `{global_clauses}/12`;").unwrap();
    writeln!(text, "- physical-body clauses: `{body_clauses}/16`;").unwrap();
    writeln!(text, "- boundary-buffer clauses: `{buffer_clauses}/8`;").unwrap();
    writeln!(
        text,
        "- total clauses: `{}/548`;",
        row_clauses + global_clauses + body_clauses + buffer_clauses
    )
    .unwrap();
    writeln!(
        text,
    ...
    .unwrap();
    writeln!(text, "\n## Physical-body clauses\n").unwrap();
    for (name, value) in CLAUSE_NAMES.into_iter().zip(body.clauses) {
        writeln!(text, "- `{name}`: `{value}`;").unwrap();
    }
    writeln!(text, "\nDetail: `{}`", body.detail).unwrap();
    writeln!(text, "\n## Boundary-buffer clauses\n").unwrap();
    for (name, value) in BUFFER_CLAUSE_NAMES.into_iter().zip(buffers.clauses) {
        writeln!(text, "- `{name}`: `{value}`;").unwrap();
    }
    writeln!(text, "\nDetail: `{}`", buffers.detail).unwrap();
    text
}
