...
fn cell_defs() -> Vec<CellDef> {
    vec![
        CellDef { name: "a", position: 0, threshold: 1 },
        CellDef { name: "b", position: 10, threshold: 1 },
        CellDef { name: "c", position: 20, threshold: 2 },
        CellDef { name: "d", position: 30, threshold: 2 },
        CellDef { name: "x", position: 40, threshold: 100 },
        CellDef { name: "y", position: 50, threshold: 100 },
        CellDef { name: "m1", position: 60, threshold: 1 },
        CellDef { name: "m2", position: 70, threshold: 1 },
        CellDef { name: "u", position: 80, threshold: 1 },
        CellDef { name: "v", position: 90, threshold: 1 },
        CellDef { name: "sink", position: 100, threshold: 100 },
    ]
}
...
impl World {
    ...
    fn run(&mut self, mut inputs: Vec<InputDef>, permutation: Permutation) -> truelearner_core::ObservedRun {
...
fn drive(name: &'static str, from: &'static str, to: &'static str, delay: i64, coupling: i32) -> ArrowDef {
    ArrowDef {
        name,
        from,
        to,
...
fn input(target: &'static str, tick: i64, impulse: i32) -> InputDef {
    InputDef { target, tick, phase: 0, impulse }
}
...
fn definitions(family: Family) -> (Vec<ArrowDef>, Vec<InputDef>) {
    match family {
        ...
        Family::MultipleModulatory => (
            vec![drive("c_x", "c", "x", 1, 1), modulation("m1_c", "m1", "c", 1), modulation("m2_c", "m2", "c", 1)],
        ...
        Family::MixedDifferentJunctions => (
            vec![drive("c_x", "c", "x", 1, 1), modulation("m1_c", "m1", "c", 1)],
            vec![input("c", 0, 2), input("m1", 0, 1), input("a", 1, 1)],
        ),
        Family::DriveCausesModulatory => (
            vec![drive("c_x", "c", "x", 1, 1), drive("a_m1", "a", "m1", 0, 1), modulation("m1_c", "m1", "c", 0)],
            vec![input("c", 0, 2), input("a", 1, 1)],
        ),
        Family::SourceFiresModulatory => (
            vec![drive("c_x", "c", "x", 1, 1), modulation("m1_c", "m1", "c", 0)],
...
fn logical_event(world: &World, event: &PhysicalEvent) -> String {
    ...
    match event {
        PhysicalEvent::DriveIncidence { target, arrivals, impulse, causal_wave } => format!("DRIVE:{}:{arrivals}:{impulse}:wave={causal_wave}", cell(*target)),
        PhysicalEvent::ModulatoryIncidence { target, arrivals, impulse, causal_wave } => format!("MOD:{}:{arrivals}:{impulse}:wave={causal_wave}", cell(*target)),
        PhysicalEvent::Deliver { mode, target, impulse } => format!("DELIVER:{mode:?}:{}:{impulse}", cell(*target)),
        PhysicalEvent::Fire { cell: id } => format!("FIRE:{}", cell(*id)),
        PhysicalEvent::Resistance { arrow: id, before, after } => format!("R:{}:{before}:{after}", arrow(*id)),
        PhysicalEvent::Coupling { arrow: id, before, after } => format!("C:{}:{before}:{after}", arrow(*id)),
        PhysicalEvent::Deallocate { arrow: id } => format!("DEALLOCATE:{}", arrow(*id)),
        PhysicalEvent::CellDeallocate { cell: id, before_generation, after_generation } => format!("CELL_DEALLOCATE:{}:{}:{}", cell(*id), before_generation.0, after_generation.0),
        PhysicalEvent::CellProposal { cell: id, source, target } => format!("CELL_PROPOSAL:{}:{}:{}", cell(*id), cell(*source), cell(*target)),
        PhysicalEvent::Proposal { arrow: id, from, to } => format!("PROPOSAL:{}:{}:{}", arrow(*id), cell(*from), cell(*to)),
        PhysicalEvent::Crossing(crossing) => format!("CROSSING:{}:{}:{}", crossing.from_region, crossing.to_region, crossing.impulse),
...
fn normalized_trace(world: &World, trace: &[PhysicalTransition]) -> Vec<String> {
    ...
    for transition in trace {
        moments.entry((transition.tick, transition.phase)).or_default().push(logical_event(world, &transition.event));
    }
    moments.into_iter().map(|((tick, phase), mut events)| {
        events.sort();
...
fn body_hash(world: &World) -> String {
    ...
    let mut rows = body.cells.iter().map(|cell| format!("CELL:{}:{}:{}:{}:{}:{}", world.cell_names[&cell.id], cell.position, cell.threshold, cell.resistance, cell.generation.0, cell.live)).collect::<Vec<_>>();
    rows.extend(body.arrows.iter().map(|arrow| format!("ARROW:{}:{}:{}:{}:{}:{}:{}:{}:{}", world.arrow_names.get(&arrow.id).copied().unwrap_or("generated_link"), world.cell_names.get(&arrow.from.id).copied().unwrap_or("generated"), world.cell_names.get(&arrow.to.id).copied().unwrap_or("generated"), arrow.delay, arrow.phase, arrow.coupling, arrow.resistance, arrow.transmission_mode, arrow.live)));
    rows.sort();
...
fn counts(world: &World, trace: &[PhysicalTransition]) -> PhysicalCounts {
    ...
    for transition in trace {
        match &transition.event {
            PhysicalEvent::DriveIncidence { arrivals, causal_wave, .. } => { counts.drive_deliveries = counts.drive_deliveries.saturating_add(u64::from(*arrivals)); counts.maximum_wave = counts.maximum_wave.max(*causal_wave); }
            PhysicalEvent::ModulatoryIncidence { arrivals, causal_wave, .. } => { counts.modulatory_deliveries = counts.modulatory_deliveries.saturating_add(u64::from(*arrivals)); counts.maximum_wave = counts.maximum_wave.max(*causal_wave); }
            PhysicalEvent::Fire { cell } => *counts.fires.entry(world.cell_names[cell].into()).or_default() += 1,
            ...
            PhysicalEvent::Proposal { .. } | PhysicalEvent::CellProposal { .. } => counts.proposals += 1,
            PhysicalEvent::Deallocate { .. } | PhysicalEvent::CellDeallocate { .. } => counts.deallocations += 1,
...
fn checks(family: Family, counts: &PhysicalCounts, quiescent: bool, ceiling: bool) -> Vec<(String, bool)> {
    let mut result = vec![("naturally_quiescent".into(), quiescent && !ceiling)];
    match family {
        Family::MultipleDrive => result.extend([("signed_drive_combined".into(), counts.drive_deliveries == 2), ("subthreshold_no_fire".into(), fire_count(counts, "c") == 0)]),
        Family::MultipleModulatory => result.extend([("two_modulatory_arrivals".into(), counts.modulatory_deliveries == 2), ("modulation_does_not_fire".into(), fire_count(counts, "c") == 1), ("multiplicity_retained".into(), counts.resistance_updates == 2)]),
        Family::MixedSameJunction => result.extend([("preparticipating_contact_updates".into(), counts.resistance_updates == 1), ("mixed_drive_fires_once".into(), fire_count(counts, "c") == 2), ("fresh_same_wave_participation_not_visible".into(), fire_count(counts, "d") == 1)]),
        Family::MixedDifferentJunctions => result.extend([("different_junction_drive_executes".into(), fire_count(counts, "a") == 1), ("different_junction_modulates".into(), counts.resistance_updates == 1)]),
        Family::DriveCausesModulatory => result.extend([("drive_chain_fires_modulator".into(), fire_count(counts, "m1") == 1), ("caused_modulation_updates".into(), counts.resistance_updates == 1), ("causal_waves_advance".into(), counts.maximum_wave >= 2)]),
        Family::SourceFiresModulatory => result.extend([("ordinary_modulator_fires".into(), fire_count(counts, "m1") == 1), ("ordinary_modulatory_arrow_updates".into(), counts.resistance_updates == 1), ("modulation_next_wave".into(), counts.maximum_wave >= 1)]),
        Family::PqlcContinuation | Family::ZeroDelayPqlcChain => result.extend([("pqlc_traverses".into(), counts.qlp_traversals == 1), ("both_contacts_update".into(), counts.resistance_updates == 2), ("pqlc_next_wave".into(), counts.maximum_wave >= 2)]),
        Family::ZeroDelayDriveChain => result.extend([("chain_fires".into(), fire_count(counts, "a") == 1 && fire_count(counts, "b") == 1 && fire_count(counts, "c") == 1), ("chain_waves".into(), counts.maximum_wave == 2)]),
        Family::ZeroDelayFanoutMerge | Family::InsertionOrder => result.extend([("fanout_fires_once".into(), fire_count(counts, "b") == 1 && fire_count(counts, "u") == 1), ("merge_combines_once".into(), fire_count(counts, "c") == 1), ("merge_wave".into(), counts.maximum_wave == 2)]),
        Family::RecurrentDrive | Family::HandleRenaming => result.extend([("cycle_each_fires_once".into(), fire_count(counts, "a") == 1 && fire_count(counts, "b") == 1), ("refractory_stops_cycle".into(), counts.maximum_wave == 2)]),
        Family::MixedRecurrence => result.extend([("mixed_cycle_settles".into(), fire_count(counts, "a") == 1 && fire_count(counts, "b") == 1), ("mixed_modulation_updates".into(), counts.resistance_updates == 1), ("mixed_wave_progresses".into(), counts.maximum_wave >= 2)]),
    }
    result
}

fn observe(root: u64, family: Family, permutation: Permutation, mechanics: MechanicalConfig) -> Observation {
    ...
    let mut restored = PlasticSubstrate::from_live_checkpoint_with_mechanics(checkpoint, world.mechanics).unwrap();
    restored.set_physical_tracing(true);
    restored.enter(SpikeInput { arrival_tick: restored.clock().tick.saturating_add(1), phase: 0, origin_physical: 199_000_000, target: world.cell("sink"), impulse: 1 });
    let continuation = restored.propagate_with_observation_ceiling(CEILING);
    let continuation_hash = ContentHash::of(format!("{:?}|{:?}|{}|{}|{}", normalized_trace(&world, &continuation.run.physical_trace), continuation.run.work, restored.clock().tick, body_hash(&World { body: restored, mechanics: world.mechanics, cells: world.cells.clone(), cell_names: world.cell_names.clone(), arrows: world.arrows.clone(), arrow_names: world.arrow_names.clone(), physical_names: world.physical_names.clone() }), continuation.run.naturally_quiescent).as_bytes()).to_string();
    Observation {
        ...
        checks: checks(family, &physical_counts, observed.run.naturally_quiescent, observed.observation_ceiling_reached),
    }
}

fn mechanics_name(mechanics: MechanicalConfig) -> &'static str {
    if mechanics == MechanicalConfig::REFERENCE { "reference" } else { "production" }
}

fn main() {
    let output_dir = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| PathBuf::from("experiments/results/ws0_complete_causal_wave_semantics_v1"));
    ...
    for root in ROOTS {
        for family in Family::ALL {
            let baseline_reference = observe(root, family, Permutation::Identity, MechanicalConfig::REFERENCE);
            let baseline_production = observe(root, family, Permutation::Identity, MechanicalConfig::PRODUCTION);
            for permutation in Permutation::ALL {
                cases += 1;
                let reference = observe(root, family, permutation, MechanicalConfig::REFERENCE);
                let reference_replay = observe(root, family, permutation, MechanicalConfig::REFERENCE);
                let production = observe(root, family, permutation, MechanicalConfig::PRODUCTION);
                let production_replay = observe(root, family, permutation, MechanicalConfig::PRODUCTION);
                let mechanics_equal = reference == production;
                let permutation_equal = reference == baseline_reference && production == baseline_production;
                for (mechanics, observation, replay) in [(MechanicalConfig::REFERENCE, &reference, &reference_replay), (MechanicalConfig::PRODUCTION, &production, &production_replay)] {
                    rows += 1;
                    let replay_equal = observation == replay;
                    let row_pass = observation.passed() && replay_equal && mechanics_equal && permutation_equal;
                    let row_clauses = observation.checks.len() + 3;
                    clauses += row_clauses;
                    passed += observation.checks.iter().filter(|(_, pass)| *pass).count() + usize::from(replay_equal) + usize::from(mechanics_equal) + usize::from(permutation_equal);
                    maximum_work = maximum_work.max(observation.work.physical_total());
                    all_pass &= row_pass;
                    let trace_hash = ContentHash::of(observation.trace.join("\n").as_bytes()).to_string();
