...
fn frame() -> Vec<u8> {
    ...
    for nonce in 0_u16..u16::MAX {
        ...
        let context = spatial_context(&candidate).expect("valid E22 frame");
        if contexts.insert(context) {
            return candidate;
        }
    }
    panic!("one E22 frame must exist")
}
...
fn recovered(mut organism: Arc3Sensorimotor) -> Arc3Sensorimotor {
    ...
    organism
        .advance_gap(1)
        .expect("ordinary E22 interaction recovery");
    organism
}
...
fn unresolved(root: u64, mechanics: MechanicalConfig, pixels: &[u8]) -> Arc3Sensorimotor {
    let mut organism =
        Arc3Sensorimotor::new_spatial_with_profile(root, mechanics, Core0Profile::GenericExternal)
            .expect("CORE1-B E22 body");
    for _ in 0..2 {
        let prime = organism
            .observe(pixels.to_vec(), &ACTIONS, None, false, false, &ACTIONS)
            .expect("E22 candidate prime");
        assert!(prime.action.is_none(), "E22 prime must remain unresolved");
        assert!(prime.naturally_quiescent, "E22 prime must quiesce");
        organism.clear_episode();
    }
    recovered(organism)
}
...
fn trigger(
    ...
    organism
        ...
        .unwrap_or_else(|error| panic!("E22 internal action {action} failed: {error}"))
}
...
fn run_opportunity(
    ...
    organism.enable_atomic_physical_credit_return();
    organism
        .open_decision_interaction(context)
        .expect("install and OPEN E22 interaction");
    let initial = organism
        .observe_open_decision(pixels.to_vec(), &ACTIONS, &ACTIONS)
        .expect("E22 initial junction activation");
    ...
    while attempts.len() < MAX_COMPLETIONS {
        ...
        let completion = organism
            ...
            .expect("E22 positive completion return");
        ...
        if participated == useful {
            let consequence = organism
                ...
                .expect("ordinary E22 consequence");
...
fn controls(
    ...
    closed.enable_atomic_physical_credit_return();
    let closed_observation = closed
        .observe(pixels.to_vec(), &ACTIONS, None, false, false, &ACTIONS)
        .expect("E22 CLOSED silence");

    let mut missing = initial.clone();
    missing.enable_atomic_physical_credit_return();
    missing
        .open_decision_interaction(context)
        .expect("E22 missing-completion OPEN");
    let selected = select_action(&missing, context, order, &HashSet::new())
        .expect("E22 missing-completion route");
    let missing_action = trigger(&mut missing, pixels, selected);

    let mut closed_completion = initial.clone();
    closed_completion.enable_atomic_physical_credit_return();
    closed_completion
        .install_decision_completion_junction(context)
        .expect("E22 CLOSED completion junction");
    let closed_return = closed_completion
        .return_decision_completion()
        .expect("E22 CLOSED completion");
    ...
    let after_success = closed_after_success
        .return_decision_completion()
        .expect("E22 post-success completion");
...
fn execute(seed: usize, mechanics: MechanicalConfig, root: u64) -> Observation {
    let pixels = frame();
    let context = spatial_context(&pixels).expect("E22 context");
    ...
    let opportunities: [Opportunity; 2] = opportunities
        .try_into()
        .expect("exactly two E22 opportunities");
...
fn exact(seed: usize) -> (Observation, bool, bool, bool) {
    let root = 122_000_000_u64 + seed as u64 * 100_000;
...
fn main() {
    ...
    if arguments.get(1).is_some_and(|value| value == "--preflight") {
        ...
        println!(
            "{:#?}",
            execute(seed, mechanics, 122_000_000 + seed as u64 * 100_000)
        );
        return;
    }
    ...
    eprintln!(
        "E22 P2={} mod={}|{} updates={}|{} final={:?} replay={} mechanics={}",
        credit_pass,
    ...
    assert!(credit_pass, "E22 P2 failed; P1 and matrix ineligible");

    let (hard, hard_replay, hard_mechanics, hard_pass) = exact(7);
    eprintln!(
        "E22 P1={} attempts={}/{} replay={} mechanics={}",
        hard_pass,
    ...
    assert!(hard_pass, "E22 P1 failed; matrix ineligible");

    eprintln!("CORE1_E22_ATOMIC_PARTICIPATION_BORN_CREDIT_RETURN_V1_EVIDENCE_SPENT");
    let destination = arguments.get(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from("experiments/results/core1_e22_atomic_participation_born_credit_return_v1")
    });
    fs::create_dir_all(&destination).expect("create E22 results");
    ...
    for seed in 0..SEEDS {
        ...
        eprintln!(
            "E22 seed={} useful={} attempts={}/{} mod={}/{} updates={}/{} final={:?} replay={} mechanics={} pass={}",
            seed,
            reference.useful,
    ...
    csv.flush().expect("flush E22 matrix");
    let passed = passed_rows == SEEDS && replay_all && mechanics_all;
    let report = format!(
        "# CORE1 E22 atomic participation-born credit return v1\n\n- primary seeds: `{passed_rows}/{SEEDS}`\n- exact replay: `{replay_all}`\n- Reference/Production exact: `{mechanics_all}`\n- primary matrix: `{passed}`\n"
    );
    fs::write(destination.join("report.md"), report).expect("write E22 report");
    println!(
        "CORE1_E22_ATOMIC_PARTICIPATION_BORN_CREDIT_RETURN_V1_COMPLETE seeds={passed_rows}/{SEEDS} replay={replay_all} mechanics={mechanics_all} pass={passed}"
    );
    assert!(passed, "E22 primary matrix failed; inspect streamed rows");
}
