...
fn csv_row(
    ...
    writeln!(
        ...
        case.resistance,
        case.scenario.name(),
        observation.candidate_eligible_events,
        observation.competitor_eligible_events,
        observation.candidate_updates,
        observation.competitor_updates,
        observation.proposals,
        observation.deallocations,
        u8::from(observation.final_candidate.live),
        observation.final_candidate.resistance,
        observation.final_candidate.coupling,
        u8::from(observation.final_candidate.eligible),
        u8::from(competitor.live),
        competitor.resistance,
        competitor.coupling,
        u8::from(competitor.eligible),
        observation.final_tick,
        observation.final_pressure_phase,
        observation.final_body_hash,
        observation.final_checkpoint_hash,
        observation.physical_trace_hash,
        u8::from(observation.naturally_quiescent),
        u8::from(observation.replay_exact),
        observation.candidate_trajectory,
        observation.competitor_trajectory,
    )
    .unwrap();
}
