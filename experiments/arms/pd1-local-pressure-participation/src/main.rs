...
fn arrow_state(body: &PlasticSubstrate, id: ArrowId) -> ArrowState {
    ...
    ArrowState {
        ...
        eligible_until: body.local_eligible_until(id),
...
fn assert_family(case: CaseSpec, observation: &Observation) {
    ...
    match case.family {
        ...
        Family::RecentlyUsed => {
            ...
            assert_eq!(after.participation, 0);
            assert!(!final_state.live);
        }
        Family::Magnitude => {
            let before = &stage(observation, "before_pressure").arrows;
            assert_eq!(before[0].eligible_until, before[1].eligible_until);
            assert_eq!(before[1].eligible_until, before[2].eligible_until);
        ...
        Family::LateMaintained => {
            ...
            assert!(before.iter().all(|arrow| arrow.eligible_until.is_none()));
            ...
            let arrows = point
                ...
                .map(|arrow| {
                    format!(
                        ...
                        arrow.resistance,
                        arrow.coupling,
                        arrow.eligible_until.map_or(-1, |tick| tick),
                        arrow.participation,
                        arrow.support,
                        arrow.pressure_load,
                    )
...
fn main() {
    let output = env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("results/pd1_local_pressure_participation_v1"));
    ...
    let family_line = Family::ALL
        ...
        .join(" ");
    let report = format!(
        "# PD1 local pressure-participation result v1\n\n\
         ...
         - local load exchange candidate: `PASS`\n\
    ...
    println!("PD1_COMPLETE physical_cases={} pass=true", case_specs.len());
}
