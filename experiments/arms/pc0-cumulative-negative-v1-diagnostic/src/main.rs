...
fn main() {
    let output = env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("results/pc0_cumulative_negative_v1_diagnostic"));
    ...
    let mut magnitude_cases_at_or_above_q = 0_u64;
    let mut magnitude_contacts_at_or_above_q = 0_u64;
    let mut maximum_magnitude_participation = 0_u64;
    ...
    for (index, case) in case_specs.iter().copied().enumerate() {
        ...
        if case.family == Family::Magnitude {
            let before = &stage(&reference, "before_pressure").arrows;
            let at_or_above_q = before
                .iter()
    ...
    let family_line = Family::ALL
        ...
        .join(" ");
    let report = format!(
        "# PC0 cumulative-negative v1 diagnostic result\n\n\
         ...
         - magnitude cases with any contact at/above Q: `{magnitude_cases_at_or_above_q}/20`\n\
         - magnitude contacts at/above Q: `{magnitude_contacts_at_or_above_q}/60`\n\
         - maximum magnitude participation: `{maximum_magnitude_participation}`\n\
         - Q: `{Q}`\n\
         - all other retained causal assertions: `PASS`\n\
         - diagnostic classification input: `SERIALIZED`\n\
    ...
    println!(
        "PC0_CUMULATIVE_NEGATIVE_V1_DIAGNOSTIC physical_cases={} serialized=true",
        case_specs.len()
    );
}
