...
98. **Automaticity may remove only a physically transparent interior.**
    We saw a reused compact-body path retain its action but perform the same five meetings and three emissions forever. The solve was to let three supported returned closures retain one direct physical link backed by the two adjacent parent links. Selection can use that link only while both parents still compose and the omitted middle has no other live drive; a changed parent or newly visible middle effect falls back before the stale link fires. The motor effect and timing remain equal while internal execution work falls, tied returns earn no structural closure, checkpoint and attachment preserve the support topology, and two such internal paths still wait for their real intervening world event. The older RE0 result prevents an inflated claim: because current `Work` does not yet amortize formation, this proves one-level execution compaction, not economically complete automaticity, recursive hierarchy, continuous retrieval, or general skill learning.
...
115. **A closed screen needs global context and local detail.**
    We found the supposed full-field retina translated a 9x9 sampling window with gaze, so an edge gaze clipped the opposite side of the only screen. The correction separates a 2048-square display from unchanged body reach, keeps fixed 8x8 global means and signed 16x16 change localization, and adds a 17x17 gaze-centred fovea. Global orienting yields to foveal reach coordinates. Equal-statistic rearrangements remain detectable, opposite-edge ARC cells enter the fovea within the 32-step observation window, and version-14 morphology migrates by deterministic append. The course again reaches LiveKey; its next failure is disengagement after an inert interaction, not invisibility or an empty centroid.

116. **Looking and acting are independent physical branches.**
    We saw one retained visual focus drive both the eyes and the hand, so a dead key remained visible and forced repeated touches. The solve was one adaptive approach line per fixed 16x16 visual subregion beside the unchanged gaze branch. Touch release opens the active line for two samples; coincident screen change strengthens it, while expiry inhibits only hand approach for 32 samples before decay permits another probe. No screen change, key, action, episode, or cause identity enters the body. LiveKey and DeadKey now pass all fresh probes and controls; the next measured frontier is Sequence.
