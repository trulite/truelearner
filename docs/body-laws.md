...
- Rust types must make illegal return transitions and role-dependent field
  meanings impossible or reject them at narrow constructors.
- Verus proves the executable return transitions are one-way and terminal
  states are absorbing. Ambiguous and expired transitions retain no closed
  support. Unique physical support remains a resolver/test obligation until
  that larger executable slice is proved.
- Lean checks higher compositional claims over frozen receipts.
- Unit tests preserve concrete laws; Academy tests whether the laws are useful.
- Representative warm wave remains strictly below 25 ns; representative warm
  regression remains strictly below 10 seconds.
