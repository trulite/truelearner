use hand_downstream_work_class_localization::{PredecessorBytes, run_all};
...
fn main() {
    ...
    let control = read(
        &predecessor_dir,
        "artifacts/predecessor-and-replay-control.json",
    );
    let prefix = read(&predecessor_dir, "artifacts/shared-prefix-work.json");
    let activity = read(&predecessor_dir, "artifacts/post-choice-activity.json");
