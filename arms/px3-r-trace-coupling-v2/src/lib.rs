...
pub struct LocalActivityOpportunity {
    pub radius: i32,
    pub overlap: i64,
    pub delay: i64,
}
...
pub struct PlasticSubstrate {
    ...
    next_serial: u64,
    pressure_tick: i64,
    local_activity_opportunity: Option<LocalActivityOpportunity>,
    recent_firings: Vec<Option<i64>>,
}
...
impl PlasticSubstrate {
    ...
    pub fn with_local_activity_opportunity(opportunity: LocalActivityOpportunity) -> Self {
        assert!(opportunity.radius > 0, "local radius must be positive");
        assert!(
            opportunity.overlap >= 0,
            "local overlap must not be negative"
        );
        assert!(opportunity.delay >= 0, "local delay must not run backward");
        Self {
            local_activity_opportunity: Some(opportunity),
            ..Self::default()
        }
    }

    pub fn install_local_activity_opportunity(&mut self, opportunity: LocalActivityOpportunity) {
        assert!(
            self.pending.is_empty(),
            "local opportunity requires quiescence"
        );
        assert!(
            self.local_activity_opportunity.is_none(),
            "local opportunity is installed only once"
        );
        assert!(opportunity.radius > 0, "local radius must be positive");
        assert!(
            opportunity.overlap >= 0,
        ...
        assert!(opportunity.delay >= 0, "local delay must not run backward");
        self.local_activity_opportunity = Some(opportunity);
        self.recent_firings.fill(None);
    }
    ...
    pub fn propagate(&mut self) -> Execution {
        ...
        while !self.pending.is_empty() {
            ...
            self.propose_local_activity_arrows(source, self.tick, &mut work);
            let outgoing = self
                .arrows
                .iter()
                .enumerate()
    ...
    pub fn persistent_bytes(&self) -> usize {
        let inherited = self.cells.len() * std::mem::size_of::<Cell>()
            + self.arrows.len() * std::mem::size_of::<Arrow>();
        if self.local_activity_opportunity.is_some() {
            inherited
                + std::mem::size_of::<LocalActivityOpportunity>()
                + self.recent_firings.len() * std::mem::size_of::<Option<i64>>()
        } else {
            inherited
        }
    }
    ...
    fn propose_local_activity_arrows(&mut self, source: CellId, tick: i64, work: &mut WorkLedger) {
        let Some(opportunity) = self.local_activity_opportunity else {
            return;
        };
        let source_position = self.cells[source.0].position;
        let mut targets = self
            ...
            .filter_map(|(index, cell)| {
                ...
                let distance = cell.position.saturating_sub(source_position).abs();
                let recent = self.recent_firings[index]
                    .is_some_and(|prior| tick.saturating_sub(prior) <= opportunity.overlap);
                (target != source
                    && cell.live
                    && (1..=opportunity.radius).contains(&distance)
                    && recent)
                    .then_some((cell.physical_id, target))
            })
            .collect::<Vec<_>>();
        targets.sort_by_key(|(physical_id, _)| *physical_id);
        for (_, target) in targets {
            for (from, to) in [(source, target), (target, source)] {
                if self
                    .arrows
                    .iter()
                    .any(|arrow| arrow.live && arrow.from == from && arrow.to == to)
                {
                    continue;
                }
                let generation = u32::try_from(self.arrows.len())
                    .unwrap_or(u32::MAX)
                    .saturating_add(2);
                let arrow_id = ArrowId(self.arrows.len());
                self.arrows.push(Arrow {
                    from,
                    to,
                ...
                if from == target {
                    self.arrows[arrow_id.0].eligible_until =
                        Some(tick.saturating_add(LOCAL_WINDOW));
                    work.local_eligibility_writes += 1;
                    self.pending.push(Spike {
                        arrival_tick: tick.saturating_add(opportunity.delay),
                        phase: 0,
                        origin_physical: self.cells[from.0].physical_id,
                        target: to,
                        target_generation: self.cells[to.0].generation,
                        impulse: 1,
                        serial: self.next_serial,
                        arrow: Some((arrow_id, generation)),
                    });
                    self.next_serial = self.next_serial.wrapping_add(1);
                    work.spikes_emitted += 1;
                }
        ...
        self.recent_firings[source.0] = Some(tick);
    }
    ...
    fn state_bytes(&self, permanent_only: bool) -> Vec<u8> {
        ...
        if let Some(opportunity) = self.local_activity_opportunity {
            push_i32(&mut bytes, opportunity.radius);
            push_i64(&mut bytes, opportunity.overlap);
            push_i64(&mut bytes, opportunity.delay);
            if !permanent_only {
                for recent in &self.recent_firings {
                    push_i64(&mut bytes, recent.unwrap_or(i64::MIN));
                }
            }
        }
        bytes
