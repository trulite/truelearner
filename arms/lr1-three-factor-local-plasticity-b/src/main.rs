...
use lr1_compartmental_physical_return::{
...
const CSV_STAGE: &str = "results/.lr1_three_factor_local_plasticity_arm_b_v1.csv.staging";
const MD_STAGE: &str = "results/.lr1_three_factor_local_plasticity_arm_b_v1.md.staging";
...
fn main() {
    match env::args().skip(1).collect::<Vec<_>>().as_slice() {
        [argument] if argument == "--preflight" => {
            ...
            println!("LR1_THREE_FACTOR_LOCAL_PLASTICITY_ARM_B_PREFLIGHT_OK");
        }
        [argument] if argument == "--lr1-b" => {
            audit();
            surface();
            absent(&[CSV, MD, CSV_STAGE, MD_STAGE]);
            evidence();
...
fn evidence() {
    eprintln!("LR1_THREE_FACTOR_LOCAL_PLASTICITY_ARM_B_EVIDENCE_SPENT");
    let rows = SEEDS
        .into_iter()
...
fn observe(
    ...
    let return_crossings = count_crossings(crossings, world.namespace, 50, 70);
...
fn build(namespace: u64, reverse: bool, reflect: bool, late: bool) -> World {
    ...
    let return_compartment = cells[7];
    let arrow_specs = [
...
fn report(rows: &[Row]) -> String {
    ...
    let edges = rows
        ...
        .sum::<u64>();
    format!(
        "# LR1 three-factor local plasticity Arm B v1\n\nOutcome: **{}**.\n\n- rows: `{passed}/{}`;\n- validity clauses: `{clauses}/192`;\n- qualified accepts: `{accepts}`;\n- adjacency scans: `{edges}`;\n- exact replay: `{}`;\n- naturally quiescent: `{}`;\n- authoritative PX0 changed: `false`;\n- new arrow mode or semantic return flag: `false`;\n- Arm A/C or PX3 executed: `false`.\n",
        if passed == rows.len() {
            "LR1-B FUNCTIONAL POSITIVE"
        } else {
            "LR1-B NEGATIVE"
        },
        rows.len(),
