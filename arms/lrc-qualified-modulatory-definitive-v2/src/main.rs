...
const PROTOCOL: &str = "b83fc8051907c5ef4ffe59c40a74d5508b347352c1d60b8d9ce0058de741b051";
...
const V1_COLLAPSE: &str = "90b90956703c6abb29743ee5944954c8f154040fe59e77088e9fd039f6b47313";
const DEV_SEEDS: [u64; 16] = [
...
fn main() {
    ...
    match args.as_slice() {
        [arg] if arg == "--preflight" => {
            ...
            surface();
            absent();
            println!("LRC_QUALIFIED_MODULATORY_DEFINITIVE_V2_PREFLIGHT_OK");
        }
        [arg] if arg == "--definitive-v2" => {
            audit();
            surface();
            absent();
            eprintln!("LRC_QUALIFIED_MODULATORY_DEFINITIVE_V2_EVIDENCE_SPENT");
            evidence();
        }
...
fn audit() {
    assert_eq!(
        sha("experiments/lrc_qualified_modulatory_transmission_definitive_protocol_v2.md"),
        PROTOCOL
    );
    assert_eq!(
    ...
    assert_eq!(
        sha("experiments/lrc_definitive_v1_namespace_contamination_collapse.md"),
        V1_COLLAPSE
    );
...
fn surface() {
    ...
    let development = DEV_SEEDS
        .into_iter()
        .flat_map(|seed| CASES.map(|case| namespace(seed, case)))
        .collect::<BTreeSet<_>>();
    let authority = AUTH_SEEDS
        .into_iter()
        .flat_map(|seed| CASES.map(|case| namespace(seed, case)))
        .collect::<BTreeSet<_>>();
    assert!(development.is_disjoint(&authority));
}

fn evidence() {
    let rows = AUTH_SEEDS
        .into_iter()
        .flat_map(|seed| CASES.map(|case| replay(seed, case, true)))
        .collect::<Vec<_>>();
...
fn replay(seed: u64, case: Case, authority_allowed: bool) -> Row {
    assert!(
        authority_allowed || !AUTH_SEEDS.contains(&seed),
        "authority cell execution refused outside --definitive-v2"
    );
...
fn build_pair(ns: u64, seed: u64, dense: bool, candidate_resistance: u32) -> PairWorld {
    let mut substrate = PlasticSubstrate::new();
    let stratum = seed_position(seed);
...
fn seed_position(seed: u64) -> usize {
    DEV_SEEDS
        ...
        .or_else(|| AUTH_SEEDS.iter().position(|value| *value == seed))
        .expect("registered development or authority seed")
}

fn paths() -> (&'static str, &'static str, &'static str, &'static str) {
    (
        "results/lrc_qualified_modulatory_transmission_definitive_v2.csv",
        "results/lrc_qualified_modulatory_transmission_definitive_v2.md",
        "results/.lrc_qualified_modulatory_transmission_definitive_v2.csv.staging",
        "results/.lrc_qualified_modulatory_transmission_definitive_v2.md.staging",
    )
}
...
mod tests {
    ...
    fn registered_matrix_is_deterministic() {
        for seed in DEV_SEEDS {
            for case in CASES {
                let row = replay(seed, case, false);
    ...
    fn definitive_serialization_has_exact_shape() {
        let rows = DEV_SEEDS
            .into_iter()
            .flat_map(|seed| CASES.map(|case| replay(seed, case, false)))
            .collect::<Vec<_>>();
        assert_eq!(rows.len(), 496);
