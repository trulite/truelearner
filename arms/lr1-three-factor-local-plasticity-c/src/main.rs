...
const CSV: &str = "results/lr1_three_factor_local_plasticity_arm_c_v1.csv";
const MD: &str = "results/lr1_three_factor_local_plasticity_arm_c_v1.md";
const CSV_STAGE: &str = "results/.lr1_three_factor_local_plasticity_arm_c_v1.csv.staging";
const MD_STAGE: &str = "results/.lr1_three_factor_local_plasticity_arm_c_v1.md.staging";
...
fn main() {
    match env::args().skip(1).collect::<Vec<_>>().as_slice() {
        [argument] if argument == "--preflight" => {
            ...
            println!("LR1_THREE_FACTOR_LOCAL_PLASTICITY_ARM_C_PREFLIGHT_OK");
        }
        [argument] if argument == "--lr1-c" => {
            audit();
            surface();
            absent(&[CSV, MD, CSV_STAGE, MD_STAGE]);
            evidence();
...
fn evidence() {
    eprintln!("LR1_THREE_FACTOR_LOCAL_PLASTICITY_ARM_C_EVIDENCE_SPENT");
    let rows = SEEDS
        .into_iter()
...
fn build(namespace: u64, reverse: bool, reflect: bool, late: bool) -> World {
    ...
    let arrow_specs = [
        ...
        modulatory(return_source, p, if late { 5 } else { 1 }, 1),
...
fn csv(rows: &[Row]) -> String {
    ...
    for row in rows {
        ...
        out.push_str(&format!(
            ...
            value.path_edges,
            value.drive_deliveries,
            value.modulatory_deliveries,
...
fn report(rows: &[Row]) -> String {
    ...
    let accepts = rows
        ...
        .sum::<u64>();
    format!(
        "# LR1 three-factor local plasticity Arm C v1\n\nOutcome: **{}**.\n\n- rows: `{passed}/{}`;\n- validity clauses: `{clauses}/192`;\n- qualified accepts: `{accepts}`;\n- modulatory deliveries: `{}`;\n- exact replay: `{}`;\n- naturally quiescent: `{}`;\n- authoritative PX0 changed: `false`;\n- physical transmission-mode state added: `true`;\n- semantic return/reward flag added: `false`;\n- Arm A/B or PX3 executed: `false`.\n",
        if passed == rows.len() {
            "LR1-C FUNCTIONAL POSITIVE"
        } else {
            "LR1-C NEGATIVE"
        },
        rows.len(),
        rows.iter().map(|row| row.observation.modulatory_deliveries).sum::<u64>(),
