...
const CSV: &str = "results/pxr0_successor_readiness_v1.csv";
const MD: &str = "results/pxr0_successor_readiness_v1.md";
const CSV_STAGE: &str = "results/pxr0_successor_readiness_v1.csv.staging";
const MD_STAGE: &str = "results/pxr0_successor_readiness_v1.md.staging";

const CASES: [Case; 16] = [
    Case::new(1_075_001, false, false, 0),
    Case::new(1_075_002, false, false, 137),
    Case::new(1_075_003, false, false, 274),
    Case::new(1_075_004, false, false, 411),
    Case::new(1_075_005, true, false, 0),
    Case::new(1_075_006, true, false, 137),
    Case::new(1_075_007, true, false, 274),
    Case::new(1_075_008, true, false, 411),
    Case::new(1_075_009, false, true, 0),
    Case::new(1_075_010, false, true, 137),
    Case::new(1_075_011, false, true, 274),
    Case::new(1_075_012, false, true, 411),
    Case::new(1_075_013, true, true, 0),
    Case::new(1_075_014, true, true, 137),
    Case::new(1_075_015, true, true, 274),
    Case::new(1_075_016, true, true, 411),
];
...
fn csv(rows: &[Row]) -> String {
    let mut text = String::from("root,reverse,reflect,shift,pair_updates,pair_impulses,formation_updates,completed_outward,duplicate_outward,incomplete_outward,blocked_outward,stale_outward,stale_proposals,stale_before,stale_after,max_work,max_bytes,quiescent,replay,clauses,passed\n");
    for row in rows {
        let trial = &row.trial;
        let fields = [
...
fn markdown(rows: &[Row], globals: [bool; 8]) -> String {
    ...
    for row in rows {
        let trial = &row.trial;
        writeln!(text, "- `{}`: `pair_updates={} impulses={}|{} formation={} out={} duplicate={} incomplete={} blocked={} stale_out={} proposals={} memory={}->{} max_work={} max_bytes={} quiet={} replay={} clauses={} passed={}`", trial.case.root, trial.pair_first.updates, trial.pair_held.impulses[0], trial.pair_held.impulses[1], trial.formation.updates, trial.completed.outward, trial.duplicate.outward, trial.incomplete.outward, trial.blocked.outward, trial.stale.outward, trial.stale.proposals, trial.stale_before, trial.stale_after, trial.max_work, trial.max_bytes, row.clauses[20], row.replay, row.clauses.iter().map(bool::to_string).collect::<Vec<_>>().join("|"), row.passed).unwrap();
    }
    text
}
