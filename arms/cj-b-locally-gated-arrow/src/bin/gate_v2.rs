...
const CSV_PATH: &str = "results/cj0_b_locally_gated_arrow_gate_v2.csv";
const REPORT_PATH: &str = "results/cj0_b_locally_gated_arrow_gate_v2.md";
const CSV_STAGE: &str = "results/.cj0_b_locally_gated_arrow_gate_v2.csv.staging";
const REPORT_STAGE: &str = "results/.cj0_b_locally_gated_arrow_gate_v2.md.staging";
...
struct BoundaryTrial {
    first_proposal_tick: i64,
    first_delivery_tick: i64,
    first_proposals: u64,
    first_pressure_updates: u64,
    first_deallocations: u64,
    first_spikes_delivered: u64,
    first_generation_checks: u64,
    first_destination_firings: usize,
    first_crossings: usize,
    first_after: ArrowSummary,
    first_end_fingerprint: u64,
    replacement_tick: i64,
    replacement_delivery_tick: i64,
    replacement_proposals: u64,
    replacement_returns: u64,
    replacement_destination_firings: usize,
    replacement_crossings: usize,
    mature_after: ArrowSummary,
    mature_fingerprint: u64,
    heldout_tick: i64,
    heldout_delivery_tick: i64,
    heldout_proposals: u64,
    heldout_destination_firings: usize,
    heldout_crossings: usize,
    heldout_fingerprint: u64,
    quiescent: bool,
    work: u64,
}
...
fn train_flat_changed(
    ...
    for round in 0..20 {
        ...
        let order = if round % 2 == 0 {
            [(Duo::Ad, base), (Duo::Cb, base + variant.changed_offset)]
        } else {
            [(Duo::Cb, base), (Duo::Ad, base + variant.changed_offset)]
        };
        for (duo, tick) in order {
            let run = execute_flat(field, duo, tick, true);
...
fn build_boundary(variant: Variant) -> BoundaryField {
    let sign = if variant.mirror { -1 } else { 1 };
    let base = variant.namespace + 0x40_000;
    let positions = [0, 2 * sign, 30 * sign];
    let regions = [0, 0, 1];
    let thresholds = [3, 3, 1];
    let mut substrate = Substrate::new();
    let mut handles: [Option<CellId>; 3] = [None; 3];
    ...
    for role in order {
        ...
        handles[role] = Some(add_cell(
            &mut substrate,
            base + u64::try_from(offset).expect("small boundary role"),
            positions[role],
            regions[role],
            thresholds[role],
        ));
    }
    let [source, destination, outward] =
        handles.map(|entry| entry.expect("complete boundary matter"));
    let mut arrows = vec![
        (destination, source, 3, 1_000, 1),
        (destination, outward, 1, 1_000, 2),
    ];
    if variant.reverse_arrows {
        arrows.reverse();
    }
...
fn enter_boundary(field: &mut BoundaryField, tick: i64) {
    let contributor = SpikeInput {
        arrival_tick: tick,
        phase: -10,
        origin_physical: 0x810,
        target: field.destination,
        impulse: 2,
    };
    let trigger = SpikeInput {
        arrival_tick: tick,
        phase: 0,
        origin_physical: 0x811,
        target: field.source,
        impulse: 3,
    };
    if field.reverse_insertion {
        field.substrate.enter(trigger);
        field.substrate.enter(contributor);
    } else {
        field.substrate.enter(contributor);
        field.substrate.enter(trigger);
    }
...
fn boundary_destination_firings(field: &BoundaryField, run: &Execution) -> usize {
    let destination = field.substrate.cell_physical_id(field.destination);
    run.trace
        .iter()
        .filter(|entry| entry.fired && entry.target_physical == destination)
        .count()
}

fn boundary_trial(variant: Variant) -> BoundaryTrial {
    let mut field = build_boundary(variant);

    enter_boundary(&mut field, 8);
    let first = field.substrate.propagate();
    let first_after = field
        .substrate
        .arrows_between(field.source, field.destination);
    let first_destination_firings = boundary_destination_firings(&field, &first);

    enter_boundary(&mut field, 11);
    let replacement = field.substrate.propagate();
    let mature_after = field
        .substrate
        .arrows_between(field.source, field.destination);
    let replacement_destination_firings = boundary_destination_firings(&field, &replacement);

    enter_boundary(&mut field, 14);
    let heldout = field.substrate.propagate();
    let heldout_destination_firings = boundary_destination_firings(&field, &heldout);

    BoundaryTrial {
        first_proposal_tick: 8,
        first_delivery_tick: 10,
        first_proposals: first.work.local_structural_proposals,
        first_pressure_updates: first.work.ordinary_pressure_updates,
        first_deallocations: first.work.physical_deallocations,
        first_spikes_delivered: first.work.spikes_delivered,
        first_generation_checks: first.work.generation_checks,
        first_destination_firings,
        first_crossings: first.crossings.len(),
        first_after,
        first_end_fingerprint: first.end_fingerprint,
        replacement_tick: 11,
        replacement_delivery_tick: 13,
        replacement_proposals: replacement.work.local_structural_proposals,
        replacement_returns: replacement.work.local_return_updates,
        replacement_destination_firings,
        replacement_crossings: replacement.crossings.len(),
        mature_after,
        mature_fingerprint: replacement.end_fingerprint,
        heldout_tick: 14,
        heldout_delivery_tick: 16,
        heldout_proposals: heldout.work.local_structural_proposals,
        heldout_destination_firings,
        heldout_crossings: heldout.crossings.len(),
        heldout_fingerprint: heldout.end_fingerprint,
        quiescent: first.naturally_quiescent
            && replacement.naturally_quiescent
            && heldout.naturally_quiescent,
        work: first.work.total() + replacement.work.total() + heldout.work.total(),
    }
}

fn run_boundary(variant: Variant) -> BoundaryResult {
    let trial = boundary_trial(variant);
    let duplicate_equal = trial == boundary_trial(variant);
    BoundaryResult {
        trial,
        duplicate_equal,
    }
...
fn run_row(variant: Variant) -> Row {
    ...
    let boundary = run_boundary(variant);
    let g0 = true;
    ...
    let boundary_trial = &boundary.trial;
    let g10 = boundary_trial.first_proposal_tick == 8
        && boundary_trial.first_delivery_tick == 10
        && boundary_trial.first_proposals == 1
        && boundary_trial.first_pressure_updates == 3
        && boundary_trial.first_deallocations == 1
        && boundary_trial.first_spikes_delivered == 3
        && boundary_trial.first_generation_checks == 3
        && boundary_trial.first_destination_firings == 0
        && boundary_trial.first_crossings == 0
        && boundary_trial.first_after.records == 1
        && boundary_trial.first_after.live == 0
        && boundary_trial.replacement_tick == 11
        && boundary_trial.replacement_delivery_tick == 13
        && boundary_trial.replacement_proposals == 1
        && boundary_trial.replacement_returns == 1
        && boundary_trial.replacement_destination_firings == 1
        && boundary_trial.replacement_crossings == 1
        && boundary_trial.mature_after.records == 2
        && boundary_trial.mature_after.live == 1
        && boundary_trial.mature_after.resistance_max == 4
        && boundary_trial.mature_after.coupling_max == 2
        && boundary_trial.heldout_tick == 14
        && boundary_trial.heldout_delivery_tick == 16
        && boundary_trial.heldout_proposals == 0
        && boundary_trial.heldout_destination_firings == 1
        && boundary_trial.heldout_crossings == 1
        && boundary_trial.quiescent
        && boundary.duplicate_equal;
    let clauses = [g0, g1, g2, g3, g4, g5, g6, g7, g8, g9, g10];
    let total_work =
        flat.work + recursive.work + convergence.work + temporal.work + boundary_trial.work;
    Row {
        variant,
        flat,
        recursive,
...
fn csv(rows: &[Row]) -> String {
    let mut output = String::from(
        "variant,namespace,initial_spacing,changed_spacing,changed_offset,flat_initial_fp,flat_initial_acquired_fp,flat_initial_gap_fp,flat_changed_acquired_fp,flat_changed_gap_fp,flat_permanent_fp,training_ab,training_cd,trained_ab,trained_cd,crossed_ad,crossed_cb,self_crossings,self_consumptions,changed_training_ad,changed_training_cb,new_ad,new_cb,old_ab,old_cd,late_old_ab,late_old_cd,old_ab_live,old_cd_live,new_ad_live,new_ad_resistance,new_cb_live,new_cb_resistance,flat_proposals,flat_deallocations,flat_arrow_count,flat_bytes,recursive_training,recursive_level_x,recursive_level_y,recursive_level_z,recursive_full_x,recursive_full_y,recursive_full_z,recursive_full_crossings,recursive_missing_b,recursive_missing_c,recursive_missing_d,recursive_r0,recursive_r1,recursive_r2,recursive_c0,recursive_c1,recursive_c2,recursive_fp,recursive_arrows,recursive_bytes,convergent_a,convergent_b,convergent_joint,convergent_cross_a,convergent_cross_b,convergent_cross_joint,convergent_fp,together_cross,together_tick,together_impulse,a_then_b_cross,a_then_b_fp,overlap_cross,overlap_tick,overlap_impulse,overlap_arrivals,within_cross,within_tick,within_impulse,absent_cross,absent_fp,temporal_fp,boundary_first_proposal_tick,boundary_first_delivery_tick,boundary_first_proposals,boundary_first_pressure_updates,boundary_first_deallocations,boundary_first_spikes_delivered,boundary_first_generation_checks,boundary_first_destination_firings,boundary_first_crossings,boundary_first_records,boundary_first_live,boundary_first_generation,boundary_first_fp,boundary_replacement_tick,boundary_replacement_delivery_tick,boundary_replacement_proposals,boundary_replacement_returns,boundary_replacement_destination_firings,boundary_replacement_crossings,boundary_mature_records,boundary_mature_live,boundary_mature_resistance,boundary_mature_coupling,boundary_mature_generation,boundary_mature_fp,boundary_heldout_tick,boundary_heldout_delivery_tick,boundary_heldout_proposals,boundary_heldout_destination_firings,boundary_heldout_crossings,boundary_heldout_fp,boundary_duplicate,boundary_quiescent,boundary_work,flat_duplicate,recursive_duplicate,convergent_duplicate,quiescent,total_work,G0,G1,G2,G3,G4,G5,G6,G7,G8,G9,G10,row_pass\n",
    );
    for row in rows {
        ...
        let mut values = vec![
            ...
            row.variant.changed_offset.to_string(),
            ...
            row.boundary.trial.first_proposal_tick.to_string(),
            row.boundary.trial.first_delivery_tick.to_string(),
            row.boundary.trial.first_proposals.to_string(),
            row.boundary.trial.first_pressure_updates.to_string(),
            row.boundary.trial.first_deallocations.to_string(),
            row.boundary.trial.first_spikes_delivered.to_string(),
            row.boundary.trial.first_generation_checks.to_string(),
            row.boundary.trial.first_destination_firings.to_string(),
            row.boundary.trial.first_crossings.to_string(),
            row.boundary.trial.first_after.records.to_string(),
            row.boundary.trial.first_after.live.to_string(),
            row.boundary.trial.first_after.generation_max.to_string(),
            format!("{:#018x}", row.boundary.trial.first_end_fingerprint),
            row.boundary.trial.replacement_tick.to_string(),
            row.boundary.trial.replacement_delivery_tick.to_string(),
            row.boundary.trial.replacement_proposals.to_string(),
            row.boundary.trial.replacement_returns.to_string(),
            row.boundary
                .trial
                .replacement_destination_firings
                .to_string(),
            row.boundary.trial.replacement_crossings.to_string(),
            row.boundary.trial.mature_after.records.to_string(),
            row.boundary.trial.mature_after.live.to_string(),
            row.boundary.trial.mature_after.resistance_max.to_string(),
            row.boundary.trial.mature_after.coupling_max.to_string(),
            row.boundary.trial.mature_after.generation_max.to_string(),
            format!("{:#018x}", row.boundary.trial.mature_fingerprint),
            row.boundary.trial.heldout_tick.to_string(),
            row.boundary.trial.heldout_delivery_tick.to_string(),
            row.boundary.trial.heldout_proposals.to_string(),
            row.boundary.trial.heldout_destination_firings.to_string(),
            row.boundary.trial.heldout_crossings.to_string(),
            format!("{:#018x}", row.boundary.trial.heldout_fingerprint),
            row.boundary.duplicate_equal.to_string(),
            row.boundary.trial.quiescent.to_string(),
            row.boundary.trial.work.to_string(),
...
fn report(rows: &[Row]) -> String {
    ...
    format!(
        ...
        rows.iter().map(|row| format!("{}|{}|{}", row.boundary.trial.first_deallocations, row.boundary.trial.mature_after.live, row.boundary.trial.heldout_crossings)).collect::<Vec<_>>().join(";")
    )
}
...
fn execute_gate() -> Result<bool, String> {
    println!("CJ0_B_LOCALLY_GATED_ARROW_GATE_V2_EVIDENCE_SPENT");
...
fn main() -> ExitCode {
    ...
    if arguments != ["--gate-v2"] {
        eprintln!("refusing execution: expected exactly --preflight or --gate-v2");
