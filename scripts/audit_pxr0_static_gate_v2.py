#!/usr/bin/env python3
"""Reconcile the immutable PXR0 runtime to all v2 static development gates."""
...
TAXONOMY = ROOT / "results/pxr0_taxonomy_v2_rerun/pxc_seam_taxonomy_summary_v2.csv"
OUTPUT = ROOT / "results/pxr0_static_gate_v2"
...
def main() -> int:
    ...
    banned_hits = []
    for surface, text in [("runtime", source_text), ("specification", spec_text)]:
    ...
    lines = [
        "# PXR0 single-file static/readiness gate v2", "",
        ...
        f"- immutable runtime SHA-256: `{runtime_sha}` (`exact={result['runtime_sha_exact']}`).", "",
        "`PXR0_STATIC_GATE_V2_OK`" if result["gate_pass"] else "`PXR0_STATIC_GATE_V2_FAIL`",
    ]
    (OUTPUT / "audit.md").write_text("\n".join(lines) + "\n")
    print(json.dumps(result, sort_keys=True))
    if result["gate_pass"]:
        print("PXR0_STATIC_GATE_V2_OK")
        return 0
    return 1
